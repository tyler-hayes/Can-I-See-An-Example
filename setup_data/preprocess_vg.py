...
# Input data
parser.add_argument('--splits_json', default=os.path.join(META_DATA_DIR, 'vg_splits.json'))
parser.add_argument('--images_json',
                    default=os.path.join(META_DATA_DIR, 'image_data.json'))
parser.add_argument('--objects_json',
                    default=os.path.join(META_DATA_DIR, 'objects.json'))
parser.add_argument('--attributes_json',
                    default=os.path.join(META_DATA_DIR, 'attributes.json'))
parser.add_argument('--object_aliases',
                    default=os.path.join(META_DATA_DIR, 'object_alias.txt'))
parser.add_argument('--relationship_aliases',
                    default=os.path.join(META_DATA_DIR, 'relationship_alias.txt'))
parser.add_argument('--relationships_json',
                    default=os.path.join(META_DATA_DIR, 'relationships.json'))
...
parser.add_argument('--output_vocab_json',
                    default=os.path.join(META_DATA_DIR, 'vocab.json'))
parser.add_argument('--output_h5_dir', default=META_DATA_DIR)
        ...
        for attribute in image['attributes']:
            ...
            try:
                for name in attribute['attributes']:
                    name = name.strip().lower()  # remove leading and trailing white space then convert to lower case
                    names.add(name)
    ...
    for split, image_ids in splits.items():
        ...
        for image_id in image_ids:
            ...
            for object_id in image_object_ids:
                image_attribute_idxs.append(len(object_attributes))
            ...
            attribute_idxs.append(image_attribute_idxs)
        ...
        numpy_arrays[split] = {
            ...
            'attribute_idxs': np.asarray(attribute_idxs),
        }
